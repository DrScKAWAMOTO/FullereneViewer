...
　きちんと証明できていませんが、この不変量は完全であると思われます。即ち、互いに同型でないフラーレンの五員環距離の分布は異なるということです。ca-generator で生成したフラーレンは TODO
...
　具体的には文字列で表します。  
　最初は空の文字列からはじまり、左側に進んだ時には 'F' を文字列の最後に追加し、右側に進んだ時には 'S' を文字列の最後に追加し、バックした時には 'b' を文字列の最後に追加します。  
