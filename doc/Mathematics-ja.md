...
　fullerene のカーボン集合から自身への錯体同型写像の数です。
...
　この二つは同型ではありません(NoAが違います)が、Pdd およびカーボン数が一致します。
...
　ある整数 threshold が与えられた時、五員環の距離が threshold 以下である五員環同士をそれぞれ一つのクラスタに分けます。これをクラスタリングと呼びます。
　整数 threshold を 0 から順番に大きくしていくと、クラスタの包含関係が出来上がります。
　このようなクラスタ一つ一つを頂点と考え、包含関係を辺と考えると、樹形図が得られます。この樹形図を「五員環のクラスタリング階層」と呼びます。

　クラスタリング階層は、ca-char --pentagon-clustering で表示できます。
...
* S1-5b6c5b6b5b
    ...
    対称度の高いフラーレンを生成します。最初の `S1` は、一つの五員環と、そのまわりに５つの六員環を並べた複体を生成の「開始状態」とすることを表します。ハイフンに続き、「開始状態」のまわりに後述する「五員環敷き詰めステップ」と「六員環敷き詰めステップ」をどのような順番で実施するかを表します。数字の `5` は「五員環敷き詰めステップ」を、数字の `6` は「六員環敷き詰めステップ」を表し、それぞれに続くアルファベットは、`a` を 0、 `b` を 1 ... `z` を 25 とみる26進数と思って数値を構成し、その数値を直前の環の敷き詰めステップの重複度とします。上記例だと「五員環敷き詰めステップ」「六員環敷き詰めステップ」「六員環敷き詰めステップ」「五員環敷き詰めステップ」「六員環敷き詰めステップ」「五員環敷き詰めステップ」の順に行います。「五員環敷き詰めステップ」とは、「開始状態」に一番近い位置（対称性が高いので、そのような位置は複数箇所あります）すべてに五員環を配置するステップです。「六員環敷き詰めステップ」とは、「開始状態」に一番近い位置（同様に対称性が高いので、そのような位置は複数箇所あります）すべてに六員環を配置するステップです。その結果、上記例だと、五員環を五個、次に六員環を五個、もう一度六員環を五個、五員環を五個、六員環を五個、五員環を一個の順です。これで球体が閉じられます。

* S2-6c5b6c5b6b

    対称度の高いフラーレンを生成します。最初の `S2` は、一つの六員環と、そのまわりに３つの五員環を一つおきに並べ、更に五員環を完全に覆いつくすように六員環を並べた複体を生成の「開始状態」とすることを表します。ハイフンに続く英数字については前出の説明通りです。

* S3-5b6c5b6c

    対称度の高いフラーレンを生成します。最初の `S3` は、一つの六員環と、そのまわりに６つの六員環を並べた複体を生成の「開始状態」とすることを表します。ハイフンに続く英数字については前出の説明通りです。

* S4-5c6e5b6d5b6b

    対称度のやや高いフラーレンを生成します。最初の `S4` は、一つの六員環と、そのまわりに６つの六員環を並べた複体を生成の「開始状態」とすることを表します。ハイフンに続く英数字についてはほぼ前出の説明通りですが、各敷き詰めステップは、対称位置が六ヶ所あっても一つおきに埋めることで対称性を少し低めています。

* S5-5d6g5c6e5b6b

    対称度が少し低めのフラーレンを生成します。最初の `S5` は、一つの六員環と、そのまわりに６つの六員環を並べた複体を生成の「開始状態」とすることを表します。ハイフンに続く英数字についてはほぼ前出の説明通りですが、各敷き詰めステップは、対称位置が六ヶ所あっても二つおきに埋めることで対称性を更に低めています。

* S6-6b5c6d5b6b

    対称度が少し低めのフラーレンを生成します。最初の `S6` は、三つの六員環と、そのまわりの凹んでいる３箇所に３つの五員環を並べ、更に五員環を完全に覆いつくすように六員環を並べた複体を生成の開始状態とすることを表します。ハイフンに続く英数字については前出の説明通りです。

* S7-5b6b5b6e5b6b5b6c

    対称度が更に低めのフラーレンを生成します。最初の `S7` は、三つの六員環と、そのまわりの凹んでいる３箇所に３つの六員環を並べた複体を生成の開始状態とすることを表します。ハイフンに続く英数字については前出の説明通りです。

* S8-5b6c5c6b5b6c5b6b

    対称度が低いフラーレンを生成します。最初の `S8` は、二つの六員環と、そのまわりの凹んでいる２箇所に２つの五員環を並べ、更に五員環を完全に覆いつくすように六員環を並べた複体を生成の開始状態とすることを表します。ハイフンに続く英数字については前出の説明通りです。

* S9-5b6b5b6e5c6b5b6e5b6b

    対称度が低いフラーレンを生成します。最初の `S9` は、二つの六員環と、そのまわりの凹んでいる２箇所に２つの六員環を並べた複体を生成の開始状態とすることを表します。ハイフンに続く英数字については前出の説明通りです。
