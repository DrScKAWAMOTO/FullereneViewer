...
* [フラーレン指定] - [C60-C82] など
    ...
    [C60](media/screenshot-0.png) をはじめとする [様々なフラーレン](media/screenshot-4.png) を選択できます。NoA はフラーレンの自己同型群の位数です。対称性の高さを表します。

* [フラーレン指定] - [extra]

    [カーボン・ナノ・チューブ](media/screenshot-5.png) や [七員環を６個使ったY-分岐フラーレン](media/screenshot-6.png) を生成します。
...
　FullereneViewer の [フラーレン指定] メニューにはこれらをリストアップしてあります。従って、炭素数101以上で対称性の無いフラーレンはリストアップできていません。
